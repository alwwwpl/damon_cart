...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		$product_info = $this->model_catalog_product->getProduct($product_id,$parent,$city,$supplier_id);
		...
		if ($product_info) {
            ...
            if ($supplier_id){
                $supplier_url = '&supplier_id='.$supplier_id;
            }
            else{
                $supplier_url = '';
            }
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('product/product', $url . '&product_id=' . $this->request->get['product_id'] . $supplier_url)
			);
			...
			if(!$this->customer->isLogged() && $this->config->get('mmos_loginseeprice_status') == '1' ){ 
				...
				$data['products'][] = array(
					...
					'tax'         => $tax,
					'rating'      => $rating,
					'href'        => $this->url->link('product/product', 'product_id=' . $result['product_id'] . $supplier)
				);
			}
