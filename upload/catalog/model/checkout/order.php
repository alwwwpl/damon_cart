...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
                /*if (empty($product['customer_id']) && empty($product['distribute_id']))
                {
                    $status = " , status = '已付款' ";
                }
                else {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', customer_id = '" . $this->db->escape($product['customer_id']) . "', supplier_id = '" . $this->db->escape($product['supplier_id']) . "', distribute_id = '" . $this->db->escape($product['distribute_id']) . "',  product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
                ...
                if ($product['bidding'] == 'bidding'){
                    $this->db->query("UPDATE " . DB_PREFIX ."bidding SET stock = 0 WHERE product_id = '" . (int)$product['product_id'] . "'");
                }
			}
		}
		...
		if (isset($data['totals'])) {
			foreach ($data['totals'] as $total) {
                if ($total['code'] == 'coupons')
                {
                    $this->db->query("UPDATE " . DB_PREFIX . "coupons_code SET order_id = '" . (int)$order_id . "', status = '1' WHERE code = '" . $total['code'] . "'");
                }
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'distribute_id'           => $order_query->row['distribute_id'],
				...
				'distribute_status'       => $order_query->row['distribute_status'],
    ...
    public function addTransaction($order_id) {
        ...
        if ($order_info['distribute_id'] > 0)
        {
            $distribute_total = $this->db->query("SELECT SUM(op.total) as total FROM `" . DB_PREFIX . "order_product` op WHERE op.order_id = '" . (int)$order_id . "' AND op.distribute_id = '" . (int)$order_info['distribute_id'] . "' ");
            if ($distribute_total->row['total'])
            {
                $this->db->query("INSERT INTO `" . DB_PREFIX . "customer_transaction` SET order_id = '" . (int)$order_id . "', customer_id = '" . (int)$order_info['distribute_id'] . "', description = '分销产品', amount = '" . $distribute_total->row['total'] . "', date_added = NOW(), cash = '0.0000'");
            }
        }
    }
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info) {
            ...
            $this->db->query("UPDATE `" . DB_PREFIX . "order_product` SET status = '已付款' WHERE order_id = '" . (int)$order_id . "' AND distribute_id = 0 AND customer_id = 0");
