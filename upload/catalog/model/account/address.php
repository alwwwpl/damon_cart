...
class ModelAccountAddress extends Model {
	public function addAddress($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$this->customer->getId() . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', company = '" . $this->db->escape($data['company']) . "', address_1 = '" . $this->db->escape($data['address_1']) . "', address_2 = '" . $this->db->escape($data['address_2']) . "', postcode = '" . $this->db->escape($data['postcode']) . "', city = '" . $this->db->escape($data['city']) . "', zone_id = '" . (int)$data['zone_id'] . "', country_id = '" . (int)$data['country_id'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "',  phone = '" . $this->db->escape(isset($data['phone']) ? $data['phone'] : '') . "'");
    ...
    public function addOrderAddress($data,$order_id) {
        $this->event->trigger('pre.customer.add.orderaddress', $data);

        $this->db->query("INSERT INTO " . DB_PREFIX . "order_address SET order_id = '" . (int)$order_id . "', customer_id = '" . (int)$this->customer->getId() . "', username = '" . $this->db->escape($data['lastname']) . "', phone = '" . $this->db->escape($data['phone']) . "', country = '" . $this->db->escape($data['country']) . "', province = '" . $this->db->escape($data['zone']) . "', city = '" . $this->db->escape($data['city']) . "', remarks = '" . $this->db->escape($data['address_1']) . "'");

        $address_id = $this->db->getLastId();

        $this->event->trigger('post.customer.add.orderaddress', $address_id);
