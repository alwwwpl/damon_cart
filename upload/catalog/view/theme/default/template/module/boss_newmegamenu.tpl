...
<nav class="mega-menu">
	...
	<ul class="nav nav-pills">
		...
		<li <?php if ((!empty($menu['menurow']))&&(count($menu['menurow']) > 0)) { echo 'class="parent"'; } ?>>
			...
			<a href="<?php echo $menu['url']; ?>"><?php if($menu['icon_class_status']){ echo '<span class="'.$menu['icon_class'].'"></span>'; }else{ if($menu['thumb']){ ?><span><img alt="<?php echo $menu['title']; ?>" src="<?php echo $menu['thumb']; ?>"/></span><?php } }?><span class="menu-title"><?php echo $menu['title']; ?></span><span class="label label-success"<?php if(!empty($menu['label_color'])){ ?>style="background-color:#<?php echo $menu['label_color']; ?>" <?php } ?> ><?php echo $menu['label']; ?></span><b>&nbsp;</b></a>
