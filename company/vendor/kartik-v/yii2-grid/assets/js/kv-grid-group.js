 ...
 */
var kvGridGroup = function (gridId) {
    "use strict";
    (function ($) {
        var $grid = $('#' + gridId), data = {}, groups = [], $groupRows, i, $tr, cols, n, colCount = 0,
            lastKey = $grid.find('tr[data-key]:last').data('key'), $pageSum = $grid.find('tr.kv-page-summary'),
            $firstRow = $grid.find('tr[data-key]:first'), $lastRow = $grid.find('tr[data-key]:last'),
            ...
            applyFormat = function (source, config, $tr, $td, i) {
                var decimals, decPoint, thousandSep, func, params;
            ...
            getParentGroup = function ($cell) {
                var $tr, $td, id = $cell.attr('data-sub-group-of'), i = 0, tag;
                if (isEmpty(id)) {
                    return null;
                }
            ...
            getSummarySource = function ($tr, $td, i) {
                var j = 1, data = [], out = 0, $row = $tr, isGrouped = $row.hasClass('kv-grid-group-row'),
        ...
        $.each(groups, function (i, g) {
            var k, gCells = data[g], rowspan = 1, gCol = 0, $gCell, $prevGroup, config,
                txtCurr = '', cellKey = '', cellKeyPrev = '', cellKeyCurr = '';
        ...
        $.each(groups, function (i, g) {
            ...
            $grid.find('td[data-group-key="' + g + '"]').each(function () {
                var $cell = $(this), $row, options = $cell.data('groupOptions'), $tr, $td, n, 
                    css = seq % 2 > 0 ? $cell.attr('data-odd-css') : $cell.attr('data-even-css');
                if (css) {
