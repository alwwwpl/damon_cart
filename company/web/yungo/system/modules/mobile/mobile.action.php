...
class mobile extends base {
	...
	public function init(){
		...
		//人气商品
		$shoplistrenqi=$this->db->GetList("select * from `@#_shoplist` where `renqi`='1' and `q_end_time` is null ORDER BY id DESC LIMIT 18");
		...
		//最新揭晓
		$shopqishu=$this->db->GetList("select * from `@#_shoplist` where `q_end_time` !='' and `q_showtime`='N' ORDER BY `q_end_time` DESC LIMIT 4");
	...
	public function glistajax(){
		...
		$sun_id_str = "'".$fen1."'";
		if($fen1){
			$sun_cate = $this->db->GetList("SELECT cateid from `@#_category` where `parentid` = '$fen1'");
		...
		if(empty($fen1)){
			...
			$brand=$this->db->GetList("select * from `@#_brand` where `cateid` in ($sun_id_str) order by `order` DESC");
	...
	public function jflistajax(){
		...
		if(empty($fen1)){
			...
			$brand=$this->db->GetList("select * from `@#_jf_brand` where `cateid` in ($sun_id_str) order by `order` DESC");
			$daohang=$this->db->GetOne("select * from `@#_category` where `cateid` = '$fen1' order by `order` DESC");
			$daohang = $daohang['name'];
		}
		...
		if($fen1){
			$count=$this->db->GetList("select * from `@#_jf_shoplist` where zongrenshu!=canyurenshu and `q_uid` is null and `cateid` in ($sun_id_str) $select_w");
		}else{
			$count=$this->db->GetList("select * from `@#_jf_shoplist` where zongrenshu!=canyurenshu and `q_uid` is null $select_w");
		}
		if($fen1){
			$shoplist=$this->db->GetList("select * from `@#_jf_shoplist` where zongrenshu!=canyurenshu and `q_uid` is null and `cateid` in ($sun_id_str) $select_w limit $star,$end");
		}else{
	...
	public function about2(){
	 $webname=$this->_cfg['web_name'];
	 $category=$this->db->GetOne("select * from `@#_category` where `parentid`='1' and `name`='购物保障'");

	 $article=$this->db->GetList("select * from `@#_article` where `cateid`='$category[cateid]'");

	include templates("mobile/index","about2");
	}
