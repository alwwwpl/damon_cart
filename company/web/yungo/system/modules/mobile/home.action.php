...
class home extends base {
    ...
    public function deladdress(){
        ...
        if(!empty($dizhi)){
            ...
            echo _messagemobile("删除失败",WEB_PATH."/mobile/home",0);
        }
    }
    ...
    public function userrecharge(){
        ...
        $paylists = $this->db->GetList("SELECT * FROM `@#_pay` where `pay_start` = '1' AND pay_class = 'yunpay'");
        // 		print_r($paylist);
    ...
    */
    ...
    public function qiandao() {
        ...
        if ( $this->segment(4) == 'mobile' ) {
            function x__messagemobile($a,$b=null,$c=2) {
        ...
        if ( isset($_REQUEST['submit']) || $this->segment(5)=='submit' ) {
            if ( !$member['mobile'] || $member['mobilecode']!='1' ) {
                x__messagemobile("非常抱歉只有手机验证会员才能签到喔",WEB_PATH."/mobile/home/qiandao/");

            }else if ( $member['sign_in_date'] == date('Y-m-d') ) {
                x__messagemobile("您今天已经过签到了。",WEB_PATH."/mobile/home/qiandao/");

            }else if ( strtotime(date('Y-m-d').$time_start ) > time() || strtotime(date('Y-m-d').$time_stop ) < time() ) {
                x__messagemobile("现在不是签到时间！签到时间为{$time_start}点到{$time_stop}点",WEB_PATH."/mobile/home/qiandao/");
                ...
                if ( $member['sign_in_date'] == date('Y-m-d',strtotime('-1 day')) ){
                    ...
                    if ( $big ) {
                        ...
                        x__messagemobile("签到成功，成功领取{$time_score}夺宝币。<br />恭喜您获得{$time_day_score}夺宝币的大礼包。<br />您的当前福分为{$score}",WEB_PATH."/mobile/home/qiandao/");
                    } else {
                        x__messagemobile("签到成功，成功领取{$time_score}福分。<br />您的当前夺宝币为{$score}。<br />再连续签到".($time_day-$sign_in_time)."天就能领取大礼包啦，加油！！！",WEB_PATH."/mobile/home/qiandao/");
                    }
                    ...
                    $mysql_model->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('".$member['uid']."', '1', '夺宝币', '每日签到', '$time_score', '".time()."')");
                    $mysql_model->Query("UPDATE `@#_member` SET score='".$score."',sign_in_time='".$sign_in_time."', sign_in_time_all='".$sign_in_time_all."', sign_in_date='".$sign_in_date."' where uid='".$member['uid']."'");
                    x__messagemobile("签到成功，成功领取{$time_score}夺宝币。<br />您的当前夺宝币为{$score}",WEB_PATH."/mobile/home/qiandao/");
                }
            }
            die;
        }
	...
	**/
    ...
    public function userphotoinsert(){
        ...
        if(isset($_POST["submit"])){
            ...
            if(!file_exists(G_UPLOAD.$tname)){_messagemobile("头像修改失败",WEB_PATH."/member/home/userphoto",3);}
            $x = (int)$_POST['x'];
            $y = (int)$_POST['y'];
            $w = (int)$_POST['w'];
            $h = (int)$_POST['h'];
            ...
            _messagemobile("头像修改成功",WEB_PATH."/member/home/userphoto",3);
...
//福分、经验添加
            ...
            _messagemobile("验证成功",WEB_PATH."/mobile/home");
        }else{
    ...
    public function userfufen(){
        ...
        include templates("mobile","userfufen");
    }
