...
class pay {
	...
	//生成订单
	private function jf_set_dingdan($pay_type='',$dingdanzhui=''){
		$uid=$this->members['uid'];
		$uphoto = $this->members['img'];
		$username = get_user_name($this->members);
		$insert_html='';
		...
		foreach($this->shoplist as $key=>$shop){
			...
			$codes = '';														//得到的购买码
			$codes_len= $shop['cart_gorenci'];													//得到购买码个数
			$money=$this->MoenyCount;										//总价格
			$status='已付款,未发货,未完成';
			$shop['canyurenshu'] = intval($shop['canyurenshu']) + $shop['cart_gorenci'];
			$shop['goods_count_num'] = $codes_len;
			$this->shoplist[$key] = $shop;
			if($codes_len){
				$insert_html.="('$dingdancode','$dingdancode_tmp','$uid','$username','$uphoto','$shop[id]','$shop[title]','$shop[qishu]','$codes_len','$money','$codes','$pay_type','$ip','$status','$time'),";
		...
		$sql="INSERT INTO `@#_member_go_jf_record` (`code`,`code_tmp`,`uid`,`username`,`uphoto`,`shopid`,`shopname`,`shopqishu`,`gonumber`,`moneycount`,`goucode`,`pay_type`,`ip`,`status`,`time`) VALUES ";
		$sql.=trim($insert_html,',');
		return $this->db->Query($sql);
	}
	...
	*	开始支付
	**/
	public function jf_go_pay($pay_checkbox){

		// if($this->pay_type=='zh'){
		// 	if($this->members['money'] < $this->MoenyCount){
		// 		return -100;
		// 	}
		// }elseif($this->pay_type=='jf'){
		// 	$fufen = System::load_app_config("user_fufen");
		// 	if($fufen['fufen_yuan']){
		// 		$this->MoenyCount = $this->MoenyCount * $fufen['fufen_yuan'];
		// 	}
		// 	if($this->members['score'] < $this->MoenyCount){
		// 		return -100;
		// 	}
		// }elseif($this->pay_type=='all'){
		// 	$fufen = System::load_app_config("user_fufen");
		// 	if($fufen['fufen_yuan']){
		// 		$this->MoenyCount = $this->MoenyCount * $fufen['fufen_yuan'];
		// 	}
	...
	//账户里支付
	private function jf_pay_bag1(){
		$time=time();
		$uid=$this->members['uid'];
		$query_1 = $this->jf_set_dingdan('账户','A');

		if($this->pay_type=='zh'){
			$pay_zhifu_name = '账户';
			$myfufen = $this->members['money'] - $this->MoenyCount;
			$query_fufen = $this->db->Query("UPDATE `@#_member` SET `money`='$myfufen' WHERE (`uid`='$uid')");
		}elseif($this->pay_type=='jf'){
			$pay_zhifu_name = '夺宝币';
			$myfufen = $this->members['score'] - $this->MoenyCount;
		...
		foreach($this->shoplist as $shop):
			if($shop['canyurenshu'] >= $shop['zongrenshu'] && $shop['maxqishu'] >= $shop['qishu']){
					$this->db->Query("UPDATE `@#_jf_shoplist` SET `canyurenshu`=`zongrenshu`,`shenyurenshu` = '0' where `id` = '$shop[id]'");
			}else{
				$shenyurenshu = $shop['zongrenshu'] - $shop['canyurenshu'];
				$query_5 = $this->db->Query("UPDATE `@#_jf_shoplist` SET `canyurenshu` = '$shop[canyurenshu]',`shenyurenshu` = '$shenyurenshu' WHERE `id`='$shop[id]'");
			}
			$goods_count_num += $shop['goods_count_num'];
		endforeach;

		$mygoscore_text =  "购买了{$goods_count_num}件商品";
		$query_2 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '-1', '$pay_zhifu_name', '$mygoscore_text', '{$this->MoenyCount}', '$time')");

		$dingdancode=$this->dingdancode;
		$query_6 = $this->db->Query("UPDATE `@#_member_go_jf_record` SET `status`='已付款,未发货,未完成' WHERE `code`='$dingdancode' and `uid` = '$uid'");
		...
		if($query_fufen && $query_1 && $query_2 && $query_5 && $query_6 && $query_7){
			$this->db->Autocommit_commit();
			foreach($this->shoplist as $shop):
				if($shop['canyurenshu'] >= $shop['zongrenshu'] && $shop['maxqishu'] >= $shop['qishu']){
					$this->db->Autocommit_start();
	...
	private function jf_pay_bag(){
		...
		if($this->MoenyCount){
			$query_4 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '-1', '账户', '购买了商品', '{$this->MoenyCount}', '$time')");
		}
		if($this->fufen){
			$query_4 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '-1', '夺宝币', '购买了商品', '{$this->fufen}', '$time')");
		}
		...
		if($query_fufen && $query_1 && $query_2 && $query_5 && $query_6 && $query_7){
			$this->db->Autocommit_commit();
			foreach($this->shoplist as $shop):
				if($shop['canyurenshu'] >= $shop['zongrenshu']){
					$this->db->Query("UPDATE `@#_jf_shoplist` SET `canyurenshu`=`zongrenshu`,`shenyurenshu` = '0' where `id` = '$shop[id]'");
				}
			endforeach;
			return true;
	...
	**/
	...
	private function pay_bag(){
		...
		if(!$this->fufen_to_money){
			...
			$myscore = $this->members['fscore'] + $mygoscore;
			$query_add_fufen_1 = $this->db->Query("UPDATE `@#_member` SET `fscore`= '$myscore' WHERE (`uid`='$uid')");
			$query_add_fufen_2 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '夺宝币', '$mygoscore_text', '$mygoscore', '$time')");
