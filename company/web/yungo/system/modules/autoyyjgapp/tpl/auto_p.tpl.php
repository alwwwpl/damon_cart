...
<span class="style2">超级人气插件使用特别注明：<br>
0、请确保有批量注册用户后方可开启（见批量注册说明）<br>
1、指定间隔时间不得太小，严重时会导致系统因数据队列过多而发生卡死。<br>
2、模块重启或关闭需要等进程结束后方可操作，需至少等待上次设置的间隔时间段(秒)的最大秒数
<br>
3、本模块仅供测试之用（可能存在线程卡死、自动退出等属正常现象、不得与其他类似模块多模块线程运行相互影响）<br>
4、请在法律范围内使用；请不要更改本插件的源代码及文件名、否则后果自负。<br>
5、本模块自动购买数量范围为不定数自动数据，<strong>默认1元，基准为1元，正常范围在100内，十几，几十偏多偶尔几百。<br/>6、注意：非正常退出停止、需要修改配置时应先点页底的停止按钮，5分钟后等线程结束再做修改保存并开启。</span><br>
</strong></span><br>
	<div class="div" >
	指定间隔时间段(秒)：
	<input class="text" id="times" type="text"   value="<?php echo $times; ?>" />（最小5秒起）
	——
		...
		<div style="width:100%; height:30px; border-bottom:1px solid #FFFFFF;">
			<table style=" background:#E1ECEC; border-left:10px solid #FFFFFF; text-align:center;">
				<tr>
					...
					<td width="60">ID</td>
					<td width="122">商品缩略图</td>
					<td width="560">商品标题</td>
		...
		<div style="width:100%;">
			<table style=" border-left:10px solid #FFFFFF; text-align:center;">
				...
				<tr>
					...
					<td width="122" ><a href="/?/goods/<?php echo $shop['id'] ?>" target="_blank" alt="查看商品详情">
<img src="/statics/uploads/<?php echo $shop['thumb']?>" width="90" height="50" /></a></td>
					<td width="560" title="<?php echo $shop['title']?>" class="style1"><?php echo $shop['title']?></td>
		...
		<div style="width:100%;">
			...
			<a href="<?php echo WEB_PATH.'/autoyyjgapp/auto_p/show/'.$o_p;?>">
				<input type="button" value="上一页" />
			</a>&nbsp;
			<a href="<?php  echo WEB_PATH.'/autoyyjgapp/auto_p/show/'.$n_p;?>">
...
$(function(){
	$("#start_auto_p").click(function(){
		...
		$.ajax({
			url:"<?php echo WEB_PATH.'/autoyyjgapp/auto_p/ajaxaction';?>",
			type:"POST",
		...
		alert("成功开起，人气模块不可多个同时进行，掉线结束属正常。");
	});
	
	$("#stop_auto_p").click(function(){
		$.ajax({
			url:"<?php echo WEB_PATH.'/autoyyjgapp/auto_p/stop';?>",
			type:"POST",
			success:function(data){
				//alert(data);
			}
		});
		alert("已关闭，请等待所有线程结束(约3分钟)");
	});
	...
	if(isstop == 0){
		$.ajax({
			url:"<?php echo WEB_PATH.'/autoyyjgapp/auto_p/errorrestart/';?>",
			type:"POST",
			success:function(data){
			}
		});
