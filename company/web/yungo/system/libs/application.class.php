...
//		define('ROUTE_C', $this->param->route_c());
        ...
        if (strpos($this->param->route_c(),'&') === false)
        {
            $route_c = $this->param->route_c();
        }
        else
        {
            $route_c = substr($this->param->route_c(),0,strpos($this->param->route_c(),'&'));
        }

        if (strpos($route_c,'/') === false)
        {
            $route_c = $route_c;
        }
        else
        {
            $route_c = substr($route_c,0,strpos($route_c,'/'));
        }
...
//        substr(ROUTE_C,0,strpos(ROUTE_C,'&'))
	}
...
//			_error($filepath,'Please verify that the path is correct.');
