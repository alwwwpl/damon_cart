...
<head>
    ...
    <title>设置您的登录密码 - <?php echo $webname; ?>触屏版</title>
	...
	<script id="pageJS" data="<?php echo G_TEMPLATES_JS; ?>/mobile/MobileCheck.js" language="javascript" type="text/javascript"></script>
</head>
<body>
...
<div class="n-h-tit"><h1 class="header-logo">设置您的登录密码</h1></div>
        ...
        <section>
		<form class="registerform" method="post" action="<?php echo WEB_PATH; ?>/mobile/user/resetpassword">
        <div class="registerCon">
	        <ul>
				<li><input name="hidKey" type="hidden" id="hidKey" value="<?php echo $key_find; ?>">
					<p class="Ptxt_F14">请重新设置您的登录密码，您的帐号是：<a class="orange Fb"><?php echo $checkcode['0']; ?></a></p>
				</li>
  ...
  <script>
	...
	$('#submitBtn').click(function(){
		...
		if(!mobileCode || /^[0-9]{6}$/.test(mobileCode) == false){
			return $.PageDialog.fail('请正确填写验证码');
		} else if(!txtPassword || txtPassword.length < 6 || txtPassword.length > 20){
			return $.PageDialog.fail('密码小于6位或大于20位');
		} else if (!txtPassword2 || txtPassword != txtPassword2) {
			return $.PageDialog.fail('两次密码输入不一致');
		}
	});
