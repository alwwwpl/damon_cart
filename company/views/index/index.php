...
?>
        ...
        <div id="focus" class="focus">
            ...
            <div class="bd">
                <ul>
                    <li><a href="#"><img src="<?php echo Yii::$app->request->baseUrl;?>/images/banner1.jpg"/></a></li>
                    <li><a href="#"><img src="<?php echo Yii::$app->request->baseUrl;?>/images/banner2.jpg"/></a></li>
                    <li><a href="#"><img src="<?php echo Yii::$app->request->baseUrl;?>/images/banner3.jpg"/></a></li>
                    <li><a href="#"><img src="<?php echo Yii::$app->request->baseUrl;?>/images/banner4.jpg"/></a></li>
    ...
    <div class="recommend openwebview clearfix">
        <h3>每周精选<a href="/category">更多</a></h3>
                ...
                if (!empty($orderProductData))
                {
                    foreach ($orderProductData as $orderProduct)
                    {
                        ...
                        <a href="./product?product_id=<?= $orderProduct['product_id']; ?>&supplier_id=<?= $orderProduct['supplier_id']; ?><?= !empty($orderProduct['distribute_id']) ? '&distribute_id='.$orderProduct['distribute_id'] : '' ?><?= !empty($orderProduct['bidding_id']) ? '&bidding_id='.$orderProduct['bidding_id'] : '' ?>">
                            <div class="img160" style="background-image: none;"><img src="http://cadmin.iddmall.com/image/<?= $orderProduct['image']; ?>"></div>
                            <span class="name"><?= $orderProduct['name'];?></span>
                            <span class="price">￥<?= Product::getProductCategory($orderProduct['product_id']) ? round($orderProduct['price'],2)*100 .'/g' : round($orderProduct['price'],2)?></span>
                        </a>
                        <?php
                    }
    ...
    <div class="newproudct openwebview clearfix">
        <h3>新品上架<a href="/category">更多</a></h3>
        <div class="pxui-content clearfix">
            <ul>
                <?php
                if (!empty($newProductData))
                {
                    ...
                    <li class="clearfix">
                        <a href="./product?product_id=<?= $newProduct['product_id']; ?>&supplier_id=<?= $newProduct['supplier_id']; ?><?= !empty($newProduct['distribute_id']) ? '&distribute_id='.$newProduct['distribute_id'] : '' ?><?= !empty($newProduct['bidding_id']) ? '&bidding_id='.$newProduct['bidding_id'] : '' ?>">
                            <div class="img160"><img src="http://cadmin.iddmall.com/image/<?= $newProduct['image']; ?>"></div>
                            <span class="name"><?= $newProduct['name'];?></span>
                            <span class="price">￥<?= Product::getProductCategory($newProduct['product_id']) ? round($newProduct['price'],2)*100 .'/g' : round($newProduct['price'],2)?></span>
                        </a>
    ...
    <div class="pin-recom openwebview clearfix">
        <h3>平台推荐<a href="/category">更多</a></h3>
        <div class="pxui-content clearfix">
            <ul>
                <?php
                if (!empty($recommendProductData))
                {
                    foreach ($recommendProductData as $recommendProduct)
                {
                    ...
                    <li class="clearfix">
                        <a href="./product?product_id=<?= $recommendProduct['product_id']; ?>&supplier_id=<?= $recommendProduct['supplier_id']; ?><?= !empty($recommendProduct['distribute_id']) ? '&distribute_id='.$recommendProduct['distribute_id'] : '' ?><?= !empty($recommendProduct['bidding_id']) ? '&bidding_id='.$recommendProduct['bidding_id'] : '' ?>">
                            <div class="img160" style="background-image: none;"><img src="http://cadmin.iddmall.com/image/<?= $recommendProduct['image']; ?>"></div>
                            <span class="name"><?= $recommendProduct['name'];?></span>
                            <span class="price">￥<?= Product::getProductCategory($recommendProduct['product_id']) ? round($recommendProduct['price'],2)*100 .'/g' : round($recommendProduct['price'],2)?></span>
                        </a>
