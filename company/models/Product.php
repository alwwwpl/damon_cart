    ...
    public function getProducts($data = array())
    {
        ...
        foreach ($productData as $result)
        {
            if (isset($result['supplier_id']) && !empty($result['supplier_id'])){
                $product_data[$result['product_id'].'_'.$result['supplier_id']] = self::getProduct($result['product_id'],$result['supplier_id'],'',Yii::$app->user->identity->parent_id,'');
            }
            else{
                $product_data[$result['product_id']] = self::getProduct($result['product_id'],'','',Yii::$app->user->identity->parent_id,'');
            }
        }
