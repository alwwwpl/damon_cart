...
<div class="n-h-tit"><h1 class="header-logo">晒单详细</h1></div>
</div>
</header>
...
<!-- 内页顶部 -->
        ...
        <section class="clearfix g-share-lucky">        
			<!-- <s class="z-Reward"></s> -->
			<a class="fl u-lucky-img" href="<?php echo WEB_PATH; ?>/mobile/mobile/userindex/<?php echo $shaidan['sd_userid']; ?>"><img border="0" alt="" src="<?php echo G_UPLOAD_PATH; ?>/<?php echo get_user_key($member['uid'],'img'); ?>"> </a>
			<div class="u-lucky-r">
				<p>幸运获得者：<a href="<?php echo WEB_PATH; ?>/mobile/mobile/userindex/<?php echo $shaidan['sd_userid']; ?>" class="z-user blue"><?php echo userid($shaidan['sd_userid'],'username'); ?></a></p>
				<?php 
				$jikxiao = get_shop_if_jiexiao($shaidan['sd_shopid']);	
				$q_end_time=date("Y-m-d H:i:s",$jikxiao['q_end_time']);
				 ?>
				<?php if($jikxiao['q_uid']): ?>
				<p>幸运<?php echo _cfg('web_name_two'); ?>码：<em class="orange"><?php echo $jikxiao['q_user_code']; ?></em></p>
				<?php endif; ?>
				<p>本期<?php echo _cfg('web_name_two'); ?>：<em class="orange"><?php echo get_user_goods_num($shaidan['sd_userid'],$shaidan['sd_shopid']); ?></em>人次</p>
				<p>揭晓时间：<em class="arial"><?php echo $q_end_time; ?></em></p> 
