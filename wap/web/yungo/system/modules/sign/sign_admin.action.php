...
class sign_admin extends admin {
	...
	public function __construct(){
		...
		$this->ment=array(
			array("lists","签到管理",ROUTE_M.'/'.ROUTE_C."/lists"),
	...
	public function lists(){
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `@#_sign` a LEFT JOIN `@#_member` b ON a.uid=b.uid WHERE $list_where");
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");
		$arr=$this->db->GetPage("SELECT a.*,b.username,b.email,b.mobile FROM `@#_sign` a LEFT JOIN `@#_member` b ON a.uid=b.uid WHERE $list_where ORDER BY id DESC", array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
