   ...
   // 今日揭晓商品
	...
	public function userlogin(){
		...
		if(!$member){
            ...
            $email = base64_encode($username);

            $password2 = base64_encode($pwd);

            $url = 'http://wap.iddmall.com/login/ajax-login?email='.$email.'&password='.$password2;
            ...
            if ($result->status == 'success')
            {
                ...
                $member = $this->db->GetOne("select * from `@#_member` where `$logintype`='$name' and `password`='$pass'");
...
//		}
	...
	public function checkname(){
		...
		if(is_array($member)){
            ...
            $email = base64_encode($name);

            $url = 'http://wap.iddmall.com/login/ajax-get-customer?email='.$email;
...
# 我的修改
		...
		$decode = 0;
		//邮箱验证 -1 代表未验证， 1 验证成功 都不等代表等待验证
		$sql="INSERT INTO `@#_member`(`mobile`,password,img,emailcode,mobilecode,yaoqing,time,username,money)VALUES('$name','$pass','$img','-1','-1','$decode','$time','$username',3)";
		if($this->db->Query($sql))
        {
            ...
            $password = base64_encode(isset($_GET['password'])? $_GET['password']: base64_decode($this->segment(5)));

            $url = 'http://wap.iddmall.com/login/ajax-register?email='.$email.'&password='.$password;

            $result = $this->GetData($url);

            $member = $this->db->GetOne("select * from `@#_member` where `mobile`='$name' and `password`='$pass'");
            ...
            if ($member)
            {
                ...
                $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '账户', '注册奖励', '3', '$go_user_time') ");
	...
	public function getLotteryList(){
	   ...
	   $isCount=$this->segment(6);
	      ...
	      $shopsum=$this->db->GetList("select * from `@#_shoplist` where `q_uid` is not null and `q_showtime` = 'N'");
