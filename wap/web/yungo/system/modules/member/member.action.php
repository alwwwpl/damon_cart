...
class member extends admin {
    ...
    public function __construct(){
        ...
        $this->ment=array(
            ...
            array("insert","每日福利",ROUTE_M.'/'.ROUTE_C."/everyday")
    ...
    */
    ...
    public function everyday()
    {
        ...
        {
            $operation = isset($_POST['operation']) ? $_POST['operation'] : 'admin';

            $money = isset($_POST['money']) ? $_POST['money'] : 0;

            $fufeng = isset($_POST['fufeng']) ? $_POST['fufeng'] : 0;

            $jingyan = isset($_POST['jingyan']) ? $_POST['jingyan'] : 0;

            $groupid = isset($_POST['groupid']) ? $_POST['groupid'] : 0;

            if ($groupid)
            {
                $result = $this->db->Getlist("SELECT uid FROM `@#_member` WHERE groupid = '".$groupid."'");
                if (!empty($result))
                {
                    ...
                    foreach ($result as $member)
                    {
                        ...
                        $this->db->Query("UPDATE `@#_member` SET money = money + ".$money.", jingyan = jingyan + ".$jingyan.", score = score + ".$fufeng." WHERE uid = ".$member['uid']);

                        if ($money)
                        {
                            $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) value ('".$member['uid']."', 1, '账户', '活动福利', '".$money."', '".$time."')");
                        }

                        if ($fufeng)
                        {
                            $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) value ('".$member['uid']."', 1, '福分', '活动福利', '".$fufeng."', '".$time."')");
                        }
                ...
                $this->db->Query("INSERT INTO `@#_welfare` (`operation`, `money`, `fufeng`, `jingyan`, `groupid`) value ('".$operation."', '".$money."', '".$fufeng."', '".$jingyan."', '".$groupid."')");
        ...
        $total = $this->db->GetCount("SELECT COUNT(*) FROM `@#_welfare` ORDER BY create_time DESC");
        ...
        $welfares = $this->db->GetPage(
            "SELECT w.*, g.name FROM `@#_welfare` as w LEFT JOIN `@#_member_group` g ON w.groupid = g.groupid ORDER BY create_time DESC",
            array(
                "num" => $num,
                "page" => $pagenum,
                "type" => 1,
        ...
        include $this->tpl(ROUTE_M,'member.everyday');
    }
