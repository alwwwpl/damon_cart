...
class cart extends base {
	...
	public function addmoney(){
		...
		public function addmoneydk(){
		parent::__construct();	
		if(!isset($_POST['submit'])){	
			_message("正在返回点卡充值页面...",WEB_PATH.'/member/home/userrechargedk');
			exit;
		}	
		if(!$this->userinfo)$this->HeaderLogin();		
		$pay_type_bank=isset($_POST['pay_bank']) ? $_POST['pay_bank'] : false;
		...
		$card_password = htmlspecialchars($_POST['card_password']);		
		
		
		$uid = $this->userinfo['uid'];
		$pay=System::load_app_class('pay','pay');
		$pay->pay_type_bank = $pay_type_bank;
		$ok = $pay->initdk($uid,$pay_type_id,'addmoney_recorddk',$money,$cardType,$restrict,$card_number,$card_password);
		if($ok === 'not_pay'){
			_message("未选择支付平台");
		}
	}
	//手机版点卡充值
	public function wapaddmoneydk(){
		parent::__construct();	
		if(!isset($_POST['submit'])){	
			_message("正在返回点卡充值页面...",WEB_PATH.'/member/home/waprechargedk');
			exit;
		}	
		if(!$this->userinfo)$this->HeaderLogin();		
		$pay_type_bank=isset($_POST['pay_bank']) ? $_POST['pay_bank'] : false;
