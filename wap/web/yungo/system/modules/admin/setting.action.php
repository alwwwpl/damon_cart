...
HTML;
	...
	public function empower(){
		if(isset($_POST['dosubmit'])){
			...
			$check = @fopen("http://www.81668.net/get.php?set_code=$code","r");  
			if(!$check){  
				_message('您输入的授权码不正确!'); 
			}   
			$html="
		...
		if($code){
			echo <<<HTML
			<iframe src="/system/shouquan/index.html" width="100%" height="100%" scrolling="no"  style=" border:0px;background:#fff; text-align:center"></iframe>
HTML;
		}else{
