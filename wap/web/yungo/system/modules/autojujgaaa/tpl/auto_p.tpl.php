...
<span class="style2">超级人气插件特别注明：<br>
1、请确保有批量注册用户后方可开启（见批量注册说明）<br>
2、指定间隔时间不得太小，严重时会导致系统因数据队列过多而发生卡死。<br>
3、本模块自动购买数量范围为不定数自动数据，<strong>默认1元，基准为1~10元，偶尔十几~几十</strong>。<br>
4、注意：非正常退出停止、需要修改配置时应先点页底的停止按钮，5分钟后等线程结束再做修改保存并开启。</span><br>
...
<img src="/statics/uploads/<?php echo $shop['thumb']?>" width="90" height="50" /></a></td>
		...
		<div style="width:100%;">
			...
			<a href="<?php echo WEB_PATH.'/autojujgaaa/auto_p/show/'.$o_p;?>">
				<input type="button" value="上一页" />
			</a>&nbsp;
			<a href="<?php  echo WEB_PATH.'/autojujgaaa/auto_p/show/'.$n_p;?>">
...
$(function(){
	$("#start_auto_p").click(function(){
		...
		$.ajax({
			url:"<?php echo WEB_PATH.'/autojujgaaa/auto_p/ajaxaction';?>",
			type:"POST",
	...
	$("#stop_auto_p").click(function(){
		$.ajax({
			url:"<?php echo WEB_PATH.'/autojujgaaa/auto_p/stop';?>",
			type:"POST",
			success:function(data){
				//alert(data);
			}
	...
	if(isstop == 0){
		$.ajax({
			url:"<?php echo WEB_PATH.'/autojujgaaa/auto_p/errorrestart/';?>",
			type:"POST",
			success:function(data){
			}
		});
