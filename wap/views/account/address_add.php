...
<style>
    ...
    <div class="w-main">
        <div class="eidt-receiving">
            ...
            ?>
            <?= $form->field($model, 'province')->hiddenInput(['value' => 0]) ?>
            <?= $form->field($model, 'city')->hiddenInput(['value' => 0]) ?>
            <?= $form->field($model, 'customer_id')->hiddenInput(['value' => Yii::$app->user->identity->customer_id]) ?>
            <?= $form->field($model, 'firstname')->hiddenInput(['value' => 0]) ?>
            ...
            <div class="item item-recename clearfix">
                <span>收货人：</span><input type="text" value="<?= $model->lastname;?>" id="address-lastname" name="Address[lastname]" class="txt-input txt-phone"  placeholder="收货人姓名" >
            </div>
            ...
            <div class="item item-address clearfix">
                <span>详细地址：</span><input type="text" id="address-address_1" value="<?= $model->address_1;?>" name="Address[address_1]" class="txt-input txt-address" placeholder="详细地址">
            </div>
