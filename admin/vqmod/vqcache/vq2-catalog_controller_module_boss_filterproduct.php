		...
		if(isset($setting['boss_filterproduct_module']['tabs']))
		{
			foreach ($setting['boss_filterproduct_module']['tabs'] as $tab) {
				...
				if ($tab['type_product'] == "popular") {
					$results = $this->model_catalog_product->getPopularProducts($setting['boss_filterproduct_module']['limit'], $city, $parent);
				}
				if ($tab['type_product'] == "special") {
					...
					$results = $this->model_catalog_product->getProductSpecials($data_sort, $city, $parent);
				}
				if ($tab['type_product'] == "best_seller") {
					$results = $this->model_catalog_product->getBestSellerProducts($setting['boss_filterproduct_module']['limit'], $city, $parent);
				}
				if ($tab['type_product'] == "latest") {
					$results = $this->model_catalog_product->getLatestProducts($setting['boss_filterproduct_module']['limit'], $city, $parent);
				}
				if ($tab['type_product'] == "category") {
					...
					$results = $this->model_catalog_product->getProducts($data_sort, $city, $parent);
				}
               ...
               if(!$this->customer->isLogged() && $this->config->get('mmos_loginseeprice_status') == '1' ){ 
                 ...
                 $tax = false;
					...
					$products[] = array(
						...
						'thumb'   	 => $image,
						'name'    	 => utf8_substr(strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')), 0, $setting['boss_filterproduct_module']['character']),
						'price'   	 => $price,
						'special' 	 => $special,
						'rating'     => $rating,
