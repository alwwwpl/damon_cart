...
class ControllerBossthemesBossAdd extends Controller {
	public function cart() {
        ...
        if (isset($this->request->post['distribute_id'])) {
            $distribute_id = (int)$this->request->post['distribute_id'];
        } else {
            $distribute_id = 0;
        }
        ...
        if (isset($this->request->post['bidding_id'])) {
            $bidding_id = (int)$this->request->post['bidding_id'];
        } else {
            $bidding_id = 0;
        }
        ...
        $product_info = $this->model_catalog_product->getProduct($product_id,$distribute_id,$city,$supplier_id,$bidding_id);
		...
		if ($product_info) {
			...
			if (!$json) {
				$this->cart->add($this->request->post['product_id'], $quantity, $option, $recurring_id, $supplier_id, $distribute_id, $bidding_id);
                ...
                if (!$logged)
                {
                    $json['total'] = explode("-",$json['total'])[0];
                }
            } else {
