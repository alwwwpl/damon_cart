...
class ControllerPaymentQrcodeweipay extends Controller {
    ...
    $data['button_confirm'] = $this->language->get('button_confirm');
		...
		*/
		...
		//sign已填,商户无需重复填写
		WxPayConf_pub::$APPID = $this->config->get('qrcodeweipay_appid');
		WxPayConf_pub::$MCHID = $this->config->get('qrcodeweipay_mchid');
		WxPayConf_pub::$KEY = $this->config->get('qrcodeweipay_key');
		WxPayConf_pub::$APPSECRET = $this->config->get('qrcodeweipay_appsecret');
		$unifiedOrder->setParameter("product_id",$order_info['order_id']);//商品描述
		...
		*/
		$data['redirect'] = $this->url->link('payment/qrcodeweipay_success');
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/qrcodeweipay.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/qrcodeweipay.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/qrcodeweipay.tpl', $data);
		}
	}
...
//					if (!$order_status_id) {
					if ($order_status_id <= 1) {
                        ...
                        $this->model_checkout_order->addTransaction($order_id);
					}else{
